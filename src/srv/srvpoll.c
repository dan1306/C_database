...
void handle_client_fsm(struct dbheader_t *dbhdr, struct employee_t *employees, clientstate_t *client){
    ...
    if(client->state == STATE_HELLO){
        ...
        printf("client upgrades to STATE_MSG\n");
    }
...
void init_clients(clientstate_t* states){
    for (int i = 0; i < MAX_CLIENTS; i++) {
        states[i].fd = -1; // -1 indicates a free chat
        states[i].state = STATE_NEW;
        memset(&states[i].buffer, '\0', BUFF_SIZE);
    }
}

int find_free_slot(clientstate t* states) {
    for(int i = 0; i < MAX_CLIENTS; i++){
        if(states[i].fd == -1){
            return i;
        }
...
int find_slot_by_fd(clientstate t* states, int fd) {
    for(int i = 0; i < MAX_CLIENTS; i++){
        if(states[i].fd == fd) {
            return i;
        }
